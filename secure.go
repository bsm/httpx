...
var secureDefaults = secure.Options{
...
func init() {
	if s := fromEnv("SECURE_HTTPS_REDIRECT"); s == "false" {
		secureDefaults.SSLRedirect = false
	}
	if s := fromEnv("SECURE_HTTPS_USE_FORWARDED_PROTO"); s == "false" {
		secureDefaults.SSLProxyHeaders = nil
	}
	if s := fromEnv("SECURE_ALLOWED_HOSTS"); s != "" {
		secureDefaults.AllowedHosts = strings.Split(s, ",")
	}

	workDir, _ := os.Getwd()
	if strings.HasPrefix(workDir, "/home/") || fromEnv("CI") != "" {
		secureDefaults.IsDevelopment = true
	}
}
